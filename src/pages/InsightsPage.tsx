...
import { FileText, Loader2, Sparkles, AlertTriangle, TrendingUp, CheckCircle, XCircle, RefreshCw } from "lucide-react";
...
const CACHE_KEY = "app:insights-cache";
...
export default function InsightsPage() {
  ...
  // Auto-generate once when data is ready and no cache exists
  const hasData = (hearings && hearings.length > 0) || (comments && comments.length > 0);
  const dataReady = !hearingsLoading && !commentsLoading && hasData;

  useEffect(() => {
    if (dataReady && !insights && !hasAutoGenerated.current && !loading) {
      ...
      <div className="container py-8">
        ...
        {loading && !insights ? (
          <div className="rounded-xl border border-border bg-card p-12 shadow-card text-center">
            ...
            <h2 className="font-display text-xl font-bold text-foreground">Generating Insights...</h2>
            <p className="mt-2 text-sm text-muted-foreground">AI is analyzing hearings and citizen comments.</p>
          </div>
        ) : insights ? (
